...
export class TasksComponent implements OnInit, OnDestroy {
  ...
  filters = {
    search: '', date: '', person: '', module: '',
    status: '', priority: '', workingType: ''
  };

  persons = ['Ansari', 'Ameen', 'Kaviya', 'Rajeena', 'Rohan'];
  statuses: TaskStatus[] = ['Pending', 'Working', 'Backend Needed', 'Testing', 'Test Done', 'Rework', 'Done'];
  ...
  private applyClientFilters(): void {
    ...
    const { search, date, person, module, status, priority, workingType } = this.filters;
    ...
    if (module) result = result.filter(t => t.module?.toLowerCase().includes(module.toLowerCase()));
    if (priority) result = result.filter(t => t.priority === priority);
    if (workingType) result = result.filter(t => t.workingType === workingType);
  ...
  resetFilters(): void {
    this.filters = { search: '', date: '', person: '', module: '', status: '', priority: '', workingType: '' };
  ...
  openEditTask(task: Task): void {
    ...
    this.form = {
      date: task.date, module: task.module, page: task.page,
      description: task.description, workingType: task.workingType,
      status: task.status, person: task.person,
      priority: task.priority, remarks: task.remarks || ''
    };
    this.showModal = true;
  }
  ...
  saveTask(): void {
    ...
    request.subscribe({
      next: (res) => {
        ...
        if (this.editId) {
          this.allTasks = this.allTasks.map(t => t._id === this.editId ? savedTask : t);
        } else {
  ...
  updateStatus(task: Task, status: TaskStatus): void {
    ...
    this.openRemarkModal('Update Status', `Add remark for "${status}"...`, () => {
      this.remarkSaving = true;
      this.taskService.updateStatus(task._id, status, this.remarkText).subscribe({
        next: (res) => {
          this.allTasks = this.allTasks.map(t => t._id === task._id ? res.data : t);
          this.cache.invalidatePrefix('tasks:');
          this.applyClientFilters();
  ...
  deleteSavedAttachment(fileName: string): void {
    ...
    this.taskService.deleteAttachment(this.editId, fileName).subscribe({
      next: (res) => {
        ...
        this.allTasks = this.allTasks.map(t => t._id === this.editId ? res.data : t);
        this.cache.invalidatePrefix('tasks:');
        this.applyClientFilters();
      },
  ...
  exportCSV(): void {
    const rows = this.tasks.map(t => ({
      date: t.date, day: t.day, module: t.module, page: t.page,
      description: t.description, workingType: t.workingType,
      status: t.status, priority: t.priority, person: t.person,
      remarks: t.remarks || '', reworkCount: t.reworkCount || 0,
  ...
  private emptyForm(): TaskPayload {
    ...
    return {
      date: today, module: '', page: '', description: '',
      workingType: 'Frontend', status: 'Pending',
